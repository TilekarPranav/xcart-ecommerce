...
public class SecurityConfig {
	...
	// login/register are exempt here — there's no established session yet to
	// protect,
	...
	// /auth/logout are intentionally NOT exempt: both act on cookies, and by the
	// time
	// either is called the frontend has already picked up an XSRF-TOKEN cookie from
	// its
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http.csrf(csrf -> csrf.csrfTokenRepository(new PartitionedCookieCsrfTokenRepository())
				...
				.authorizeHttpRequests(auth -> auth.requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()
						...
						.requestMatchers(HttpMethod.PUT, "/inventory/**").hasRole("ADMIN").requestMatchers("/admin/**")
	 ...
	 * cookie would never appear at all. Forcing csrfToken.getToken() on every
	 * request is the standard Spring Security pattern for SPA CSRF integration —
